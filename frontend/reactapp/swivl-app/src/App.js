...
function validateUsername(userName, password) {
    //todo test after serverside done

    request
        .post('http://localhost:8080/api/usernameConfirm')
        .set('Content-Type', 'application/x-www-form-urlencoded')
        .send({username: userName})
        .end(function (err, res) { // should check for error instead of true or false maybe?
    ...
    request
        ...
        .end(function (err, res) { // should check for error instead of true or false maybe?
            if (err) {
                ...
                // todo change to Next view
            }
        });
}

function confirmPasswordUsername(username, password) {
    if (!validPassword()) {
        showError("Password needs to have a minimum length of 5");
        return
    }
...
class App extends Component {
    ...
    render() {
        ...
        return (
            <View>
                ...
                {/*<FormInput onChangeText={usernameChanged()}/>*/}
                ...
                <View style={styles.container}>
                    ...
                    <Button title="Refactor form" onPress={this.handleRefactor}/>
                    <Button title="Validation form" onPress={this.handleValidation}/>
                    {activeForm === 'hello' && <HelloForm/>}
                    {activeForm === 'simple' && <SimpleForm/>}
                    {activeForm === "refactor" && <RefactorForm/>}
                    {activeForm === "validate" && <ValidationForm/>}
                </View>
    ...
    handleRefactor = () => this.setState({activeForm: "refactor"});

    handleValidation = () => this.setState({activeForm: "validateform"});
}
